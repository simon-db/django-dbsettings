...
__all__ = ['Value', 'BooleanValue', 'DecimalValue', 'EmailValue',
           ...
           'PositiveIntegerValue', 'StringValue', 'TextValue','TextValueEditor',
           'MultiSeparatorValue', 'ImageValue', 'FileValue',
...
class TextValueEditor(Value):
    unitialized_value = ''

    class field(forms.CharField):
        from cked.widgets import CKEditorWidget
        class widget(CKEditorWidget):
            pass
    def to_python(self, value):
        return unicode(value)
...
class FileValue(Value):
        ...
        class widget(forms.FileInput):
            ...
            def render(self, name, value, attrs=None):
                ...
                try:
                    ...
                    file_name = pjoin(settings.MEDIA_URL, value.name).replace("\\","/")
                    params = {"file_name" : file_name }
                    output.append(u'<p><a href="%(file_name)s" />%(file_name)s</a></p>' % params )
                except IOError:
                    pass
    ...
    def to_editor(self, value):
        ...
        try:
            with open(file_name, 'rb') as f:
                uploaded_file = SimpleUploadedFile(value, f.read(), content_type='text/xml')
